...
function Coin() {
  ...
  const { isLoading: infoLoading, data: infoData } = useQuery<InfoData>(
    ["info", coinId],
    () => fetchCoinInfo(coinId), // 파라미터(coinId)를 넘기기 위해 화살표 함수로 표현한다.
    { refetchInterval: 5000 } // 5초 마다 데이터를 불러옴. 시세 변경 확인을 위해!
  );
  const { isLoading: ticktersLoading, data: tickersData } = useQuery<PriceData>(
    ["tickers", coinId],
    ...
    <Container>
      ...
      {loading ? (
        ...
        <>
          ...
          <Tabs>
            {/* isActive로 해당 탭이 선택되어있는지를 확인하여 글자색을 변경 */}
          ...
          <Switch>
            {/* 현재 URL에 따라 렌더링하는 컴포넌트 변경. chart or price */}
