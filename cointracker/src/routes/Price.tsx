...
const Percent = styled.span<{ percent?: number }>`
  color: ${(props) =>
    props.percent && props.percent > 0 ? "#2ecc71" : "#e74c3c"};
`;
  ...
  return (
    <Taps>
      ...
      <Tap>
        ...
        <Percent percent={priceData?.market_cap_change_24h}>
          {priceData?.market_cap_change_24h}%
        </Percent>
      </Tap>
      <Tap>
        <span>Change rate (last 30 Minutes) :</span>
        <Percent percent={priceData?.percent_change_30m}>
          {priceData?.percent_change_30m}%
        </Percent>
      </Tap>
      <Tap>
        <span>Change rate (last 1 hours) :</span>
        <Percent percent={priceData?.percent_change_1h}>
          {priceData?.percent_change_1h}%
        </Percent>
      </Tap>
      <Tap>
        <span>Change rate (last 12 hours) :</span>
        <Percent percent={priceData?.percent_change_12h}>
          {priceData?.percent_change_12h}%
        </Percent>
      </Tap>
      <Tap>
        <span>Change rate (last 24 hours) :</span>
        <Percent percent={priceData?.percent_change_24h}>
